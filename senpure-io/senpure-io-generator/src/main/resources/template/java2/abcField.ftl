...
<#if field.list >
    private List<${.globals[field.fieldType]!field.fieldType?cap_first}> ${field.name}=new ArrayList(<#if field.capacity gt 0>${field.capacity}</#if>);
<#else>
    private ${field.fieldType} ${field.name};
</#if>
</#list>
...
<#list fields as field>
    ...
    public ${name} set${field.index}${field.name?cap_first} (List<${.globals[field.fieldType]!field.fieldType?cap_first}> ${field.name}){
        if(${field.name} == null){
        this.${field.name} = new ArrayList(<#if field.capacity gt 0>${field.capacity}</#if>);
            return this;
        }
    ...
    public ${name} set${field.index}${field.name?cap_first}(${field.fieldType} ${field.name}) {
        this.${field.name}=${field.name};
        return this;
    }
    </#if>
