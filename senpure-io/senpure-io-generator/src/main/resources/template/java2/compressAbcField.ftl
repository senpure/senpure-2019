...
<#list fields as field>
    ...
    public ${name} set${field.index}${field.name?cap_first} (List<${.globals[field.classType]!field.classType?cap_first}> ${field.name}){
        if(${field.name} == null){
        this.${field.name} = new ArrayList(<#if field.capacity gt 0>${field.capacity}</#if>);
            return this;
        }
    ...
    public ${name} set${field.index}${field.name?cap_first}(${field.classType} ${field.name}) {
        this.${field.name}=${field.name};
        return this;
    }
    </#if>
