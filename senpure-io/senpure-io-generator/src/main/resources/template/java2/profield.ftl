...
<#if field.list >
    protected List<${.globals[field.classType]!field.classType?cap_first}> ${field.name}=new ArrayList(<#if field.capacity gt 0>${field.capacity}</#if>);
<#else>
    protected ${field.classType} ${field.name};
</#if>
</#list>
